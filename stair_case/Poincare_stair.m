function x_next = Poincare_stair(x,d,ctrl)
% d in cm, returns the next state on the Poincare section 
% S = {x \in TQ | h^v_foot(q) = d}

file = 'fixedpointforfivelink/Switching_Control/fixed_point_d=-2cm.mat';
file = replaceBetween(file,"point_d=","cm.mat",num2str(100*ctrl));
fix = open(file);
tplot = 0;
ival = fix.x;
M_bez = 6;
...
options = odeset('Events',@(t,q)touchdown5_steps(t,q,d),'RelTol',1e-5,'AbsTol',1e-4);
[t,y,te,ye,ie] = ode45(@(t,Z) return_map5_original(t,Z,Fext,tF,alpha,theta_minus,theta_plus,M_bez),[0 20],x,options);
x_next = impact_map5(ye);

for tp=1:size(t)
    [fx,gx,gforce]=fcn_stance_dynamics5(y(tp,:));
    u(tp,:)=fcn_stance_controller5(y(tp,:),fx,gx,gforce,[0;0],alpha,theta_minus,theta_plus,M_bez);
end
...
% 
% fix=open('fixedpointforfivelink/Switching_Control/fixed_point_d=-2cm.mat');
% % ival=fix.fpStates';
