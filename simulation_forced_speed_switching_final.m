...
% [~,start] = min(abs(data(:,1)-start_speed));
XX = data(start,3:12)';

% index = [4 10 20 30 40 51 62 75];
% index = [4 20 40 62];
index = 1:1:79;
zeta_star = data(index,13);
...
%         end
                ...
                Fvir(tp)=(k3(tp,:)*Fext_plot(:,tp))/k1(tp);
        ksi1(tp)=theta(tp);
    ...
    ksi2_plus = gamma0(y(1,1:5)')*y(1,6:10)';
    zeta_plus = 0.5*ksi2_plus^2;
    
    nominal_speed_record(i) = data(k,1);
    
    Wf=trapz(theta,Fvir);
    zeta_f = data(k,13) + Wf/(1-delta_zero^2);
%     if rem(i,1)==0
%         for j=1:1:size(data,1)
%             Wf(j)=trapz(theta,Fvir(j,:))/2;
%             zeta_f(j) = data(k,13) + Wf(j)/(1-delta_zero^2);
%         end
        int = 0;
        zeta_check = int*zeta_f + (1-int)*zeta_minus;
        [min_zeta,next_index] = min(abs(zeta_check-zeta_star));
        k = index(next_index);
        k=start;
...
%     [~,k] = min(abs(data(:,13)-zeta_minus));
    ...
    fprintf('i=%1.0f,\t desired_speed=%1.4f,\t average_speed=%1.4f,\t umax= %3.2f,\t friction= %2.4f, \t max_output_error= %1.5f, \t Wf=%3.2f \n',i,desired_speed,average_speed(i),u_max,fric_max,max(max(output_error(:,:))),Wf);
...
figure(1)
plot(tplot_record,Force_RMS);
figure(2)
plot(tplot_record,average_speed)
hold on
plot([0,tplot_record(end)],[vL,vL]);
figure(3)
plot([0,tplot_record(end)],[vL,vL],'r','LineWidth',2);
hold on;
for i = 1:1:length(tplot_record)
    if i==1
    plot([0,tplot_record(i)],[nominal_speed_record(i),nominal_speed_record(i)],'LineWidth',3);
    hold on;
    plot([tplot_record(i),tplot_record(i)],[nominal_speed_record(i),nominal_speed_record(i+1)],'LineWidth',3);
    elseif i==length(tplot_record)
        plot([tplot_record(i-1),tplot_record(i)],[nominal_speed_record(i),nominal_speed_record(i)],'LineWidth',3);
        hold on;
    else
        plot([tplot_record(i-1),tplot_record(i)],[nominal_speed_record(i),nominal_speed_record(i)],'LineWidth',3);
        hold on;
