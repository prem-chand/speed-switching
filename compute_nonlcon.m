function [C,Ceq]=compute_nonlcon(initial_alpha_red,Fext,tF,d)
...
%         (average_speed+0.10)/0.10];
        C=[(max(max(abs(u)))-100)/100 ; -1*min(q4) ; -1*min(q5); (max(friction_cone)-0.5)/0.5 ; 
%             (norm_output_error-1e-3)*1000; ...
            -1*(average_speed-0.40)/0.40 ; (average_speed-0.80)/0.80];
        Ceq=norm(fp-XX);
    end
end
