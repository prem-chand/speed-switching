...
beta_s = fcn_beta_correction(x_fix,x_fix,zeros(12,1));
%event=x_fix(2);
M_bez=6;
tF=[0 50];
Fext=[0  0];
...
% % %     [betta,beta_c,theta_minus,theta_plus] = fcn_alpha_red_correction(ival,x_fix,N);
% % %     [~,~,~,ye,~]=...
% % %         ode45(@(t,Z) return_map5(t,Z,[F_pert;0],tF,betta,beta_c,beta_s,theta_minus,theta_plus,M_bez,N),[0,tfin],y0,options);
% % %     y0_plus = impact_map5(ye);
% % %     
% % %     %backward perturbation
% % %     F_pert = F(i) - dF;
...
%     beta_s_p_tau;
    ...
    average_speed_plus=step_length(1)/te;
%     te_plus = te;
...
%     x_fix(i) = x_fix(i)-2*x;
    ...
    [~,~,te,ye,~]=...
        ode45(@(t,Z) return_map5(t,Z,[F;0],tF,betta,beta_c,beta_s_p_tau,theta_minus,theta_plus,M_bez,N),[0,tfin],y0,options);
    step_length=fcn_position_swingfoot(ye);
    average_speed_minus=step_length(1)/te;
%     te_plus = te;
    
    % A is the Jacobian matrix of poincare map at fixed point, compute the
    % i-th column of A
%     G(1,i) = (te_plus-te_minus)/(2*dbeta);
    G(1,i) = (average_speed_plus-average_speed_minus)/(2*dbeta);
end
G
% end
